...
async function fetchExchangeRates(
  apiKey: string,
): Promise<{ gbpUsd: number; eurUsd: number }> {
  try {
    const response = await fetch(
      "https://api.oilpriceapi.com/v1/prices/latest?by_code=GBP_USD,EUR_USD",
      {
        headers: {
    ...
    if (response.ok) {
      ...
      const gbpRate =
        prices.find((p: any) => p.code === "GBP_USD")?.price || 1.3;
      const eurRate =
        prices.find((p: any) => p.code === "EUR_USD")?.price || 1.1;
  ...
  return { gbpUsd: 1.3, eurUsd: 1.1 }; // Fallback rates
}
...
export async function createProcessSheet(): Promise<void> {
  await Excel.run(async (context) => {
    ...
    let processSheet =
      context.workbook.worksheets.getItemOrNullObject("Process");
    await context.sync();
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (commodityInfo.unit === "MBtu") {
        ...
        pricePerMBtu = convertToMBtu(
          usdPrice,
          commodityInfo.unit as any,
          heatContent,
        );
      }
    ...
    if (processRows.length > 0) {
      const processDataRange = processSheet.getRange(
        `A2:G${processRows.length + 1}`,
      );
      processDataRange.values = processRows;

      // Format columns
      processSheet.getRange(`B2:B${processRows.length + 1}`).numberFormat = [
        ["#,##0.00"],
      ]; // Original price
      processSheet.getRange(`E2:E${processRows.length + 1}`).numberFormat = [
        ["$#,##0.00"],
      ]; // USD price
      processSheet.getRange(`F2:F${processRows.length + 1}`).numberFormat = [
        ["0.000"],
      ]; // Heat content
      processSheet.getRange(`G2:G${processRows.length + 1}`).numberFormat = [
        ["$#,##0.00"],
      ]; // Price per MBtu
    }
...
/**
 * Fetch Data Connector prices (bunker fuels from BYOS sources)
 */
export async function fetchDataConnectorPrices(
  apiKey: string,
  options: DataConnectorOptions = {},
): Promise<DataConnectorPrice[]> {
  const client = new OilPriceAPIClient(apiKey);
  return await client.getDataConnectorPrices(options);
}

/**
 * Create Data Connector sheet with bunker fuel prices
 * Displays prices from connected data sources like Ship&Bunker
 */
export async function createDataConnectorSheet(
  prices: DataConnectorPrice[],
): Promise<void> {
  await Excel.run(async (context) => {
    // Get or create Bunker Prices sheet
    let sheet =
      context.workbook.worksheets.getItemOrNullObject("Bunker Prices");
    await context.sync();

    if (sheet.isNullObject) {
      sheet = context.workbook.worksheets.add("Bunker Prices");
    }
    ...
    headerRange.values = headers;
    headerRange.format.font.bold = true;
    headerRange.format.fill.color = "#e8f5e9"; // Light green for marine theme
    ...
    if (rows.length > 0) {
      const dataRange = sheet.getRange(`A2:I${rows.length + 1}`);
      dataRange.values = rows;

      // Format price column as currency
      const priceRange = sheet.getRange(`C2:C${rows.length + 1}`);
      priceRange.numberFormat = [["$#,##0.00"]];
    }
